/*************************************************************
   Lvkrn.h - LEADTOOLS vector library
...
#define VECTOR_HPOLYBEZIER                   (18)        // Hetrogenous poly bezier
...
#else
   struct _VECTORHANDLE;
   typedef L_INT ( pEXT_CALLBACK  pVECTOREVENTPROCA )(struct _VECTORHANDLE *pVector, L_UINT uLevel, pVECTOREVENTA pEvent, L_VOID *pUserData );
   #if defined(FOR_UNICODE)
      typedef L_INT ( pEXT_CALLBACK  pVECTOREVENTPROC )(struct _VECTORHANDLE *pVector, L_UINT uLevel, pVECTOREVENT pEvent, L_VOID *pUserData );
   #else
      typedef pVECTOREVENTPROCA pVECTOREVENTPROC;
   #endif// #if defined(FOR_UNICODE)
...
#else
   typedef L_INT (pEXT_CALLBACK pVECTORFONTMAPPERCALLBACKA)(struct _VECTORHANDLE* pVector, LPLOGFONTA pLogFont, L_VOID* pUserData);
#endif // #if !defined(LT_COMP_VECTOR)
...
#  else
      typedef L_INT (pEXT_CALLBACK pVECTORFONTMAPPERCALLBACK)(struct _VECTORHANDLE* pVector, LPLOGFONTW pLogFont, L_VOID* pUserData);
#  endif
#  else
...
/*[]-----------------------------------------------------------------------[]*/
#if defined(FOR_WINDOWS)
L_LVKRN_API L_BOOL EXT_FUNCTION L_VecClipboardReady();
...
#else 
  L_LVKRN_API L_INT EXT_FUNCTION L_VecSetEventCallbackA( struct _VECTORHANDLE *pVector, pVECTOREVENTPROCA pProc, L_VOID *pUserData, ppVECTOREVENTPROCA pOldProc, L_VOID **pOldUserData );
#endif // #if !defined(LT_COMP_VECTOR)
